        ...
        if (cardSelection.length == 2) {
          ...
          if (this.cards[cardSelection[0]].value !=
            ...
            if (revealedCards == this.cards.length) {
              ...
              revealedValues = [];
              status.code = 4,
              status.message = 'GAME OVER! :( prova:' + this.attempts +
                  ', gabime: ' + this.mistakes;
            }
            else {
              status.code = 2,
              status.message = 'bravo';
            }
          }
          cardSelection = [];
        }
          ...
          status.code = 1,
          status.message = 'shtyp nje leter';
        }
      }
      else {
        status.code = 0,
        status.message = 'ska nevoje ta shtypesh dy her';
      }
